...
В проекте намеренно оставлена одна основная пользовательская лаборатория атак — **Attack Lab**. В ней доступны разные версии атак: удаление узлов, удаление ребер, Ricci/flow-стратегии и смешанные rewire/replace-сценарии. Их общий backend — `AttackService`, который вызывает реализованные модели из `src.attacks` и `src.attacks_mix`. Вкладка `Layers` не является второй Attack Lab: она только запускает слойный pipeline, показывает summary/artifacts и использует тот же сервисный контракт.
